...
import com.akasia.investment.task.bank.Bank;
import com.akasia.investment.task.bank.BankDao;
import com.akasia.investment.task.company.Company;
import com.akasia.investment.task.company.CompanyDao;
import com.akasia.investment.task.service.AkasiaService;
import com.akasia.investment.task.service.FocalService;
...
public class BankTest {
    Logger log
    = LoggerFactory.getLogger(BankTest.class);
	...
	@Scheduled(fixedRateString = "${app.akasia.bank-test-frequency}")
	public void executeTask() throws IOException, JSONException {
		...
		AkasiaService akasiaService = new AkasiaService(environment, companyDao);
		FocalService focalService = new FocalService(environment, companyDao);
		...
		if (akasiaService.getResponseCode() == 200) {
			if (akasiaBank != null) {
				akasiaBankId = akasiaBank.getBankId();
				akasiaBankName = akasiaBank.getBankName();
				log.info("akasiaBankId:" + akasiaBankId);
